...
u64 key_hash(TestKey* key) { return str_hash(key->s.data); }

bool key_equal(const TestKey* lhs, const TestKey* rhs) { return string_equal(&lhs->s, &rhs->s); }
...
void test_hash_table_complex_entry(void) {
    ...
    HashTable ht = {};
    hash_table_init(&ht, &scratch, (HashFunc)key_hash, (KeyEqualFunc)key_equal, (CopyItem)key_copy,
                    (CopyItem)value_copy, (FreeItem)key_free, (FreeItem)value_free);
