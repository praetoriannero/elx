...
static const u32 valid_unary_ops[] = {
    TOK_STAR,  // *
    TOK_AMPER, // &
    TOK_BINV,  // ~
    TOK_MINUS, // -
...
static inline bool is_valid_unary_op(TokenKind kind) {
  usize table_len = array_len(valid_unary_ops);
  for (usize idx = 0; idx < table_len; idx++) {
    if (kind == valid_unary_ops[idx]) {
      return true;
    }
  }
