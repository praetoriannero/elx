...
void print_func(Func* func, u64 depth) {
  ...
  if (func->param_vec.size) {
    ...
    vector_foreach(func_arg, func->param_vec) {
      indent(depth);
      printf("param {%.*s: %.*s},\n", span_cstr(func_arg->span), span_cstr(func_arg->type.span));
    }
  }
  depth--;
  indent(depth);
...
void print_struct(Struct* struct_, u64 depth) {
  ...
  for (u32 i = 0; i < struct_->field_vec.size; i++) {
    ...
    // xnotnull(field);
    printf("field { ident: %.*s, type: %.*s },\n", span_cstr(field->span), span_cstr(field->type.span));
  }
...
void print_expr(Expr* expr, u64 depth) {
  ...
  switch (expr->expr_kind) {
    case EXPR_KIND_METHOD_CALL:
      ...
      printf("},\n");
      indent(depth);
      printf("field: %.*s,\n", span_cstr(expr->method_call_expr.method));
    ...
    case EXPR_KIND_BINARY:
      printf("binary %.*s {\n", span_cstr(expr->binary_expr.op.span));
    ...
    case EXPR_KIND_UNARY:
      printf("unary %.*s {\n", span_cstr(expr->unary_expr.op.span));
    ...
    case EXPR_KIND_FIELD:
      ...
      printf("},\n");
      indent(depth);
      printf("field: %.*s,\n", span_cstr(expr->field_expr.span));
      depth--;
      indent(depth);
      printf("},\n");
      break;
