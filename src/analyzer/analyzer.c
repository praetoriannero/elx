...
void analyzer_visit_module(Module* module, AnalyzerContext* ctx) {
  // HashTable* ast_node_set = hash_table_new(str_hash, str_equal);
...
void analyzer_visit_ast(Ast* ast, AnalyzerContext* ctx) {
  ...
  // // ctx->global_scope->symbol_table = hash_table_new(g_str_hash, g_str_equal);
  // // hash_table_init(&ctx->global_scope)

  // Module* module = NULL;
  // vector_foreach(module, ast->module_vec) { analyzer_visit_module(module, ctx); }
