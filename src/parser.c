...
Expr* parse_expr_prec(Allocator* allocator, Parser* self, TokenKind stop_token, ExprPrecedence min_prec) {
  ...
  Expr* prev_lhs;
  Token left_tok = lexer_peek(allocator, &self->lexer);
  ...
  left_tok = lexer_next(allocator, &self->lexer);
  ...
  if (left_tok.kind == TOK_IDENT) {
    ...
    if (!comma_count) {
      ...
      Expr* element_type = parse_expr_prec(allocator, self, TOK_SEMICOLON, 0);
      parser_expect(TOK_SEMICOLON, lexer_next(&local_allocator, &self->lexer));
      Expr* element_count = parse_expr_prec(allocator, self, TOK_RBRACK, 0);
    ...
    log("group expr\n");
    lhs = parse_expr_prec(allocator, self, TOK_RPAREN, 0);
  ...
  while (true) {
    // log("checking rhs\n");
    Token operator_tok = lexer_peek(allocator, &self->lexer);
    ...
    if (postfix_prec) {
      ...
      operator_tok = lexer_next(allocator, &self->lexer);
      ...
      if (operator_tok.kind == TOK_LBRACE) {
        ...
        operator_tok = lexer_next(allocator, &self->lexer); // field
        if (TOK_IDENT != operator_tok.kind) {
          panic("expected field identifier after token '.' but found '%s'", operator_tok.str.data);
        }

        Token lparen = lexer_peek(allocator, &self->lexer); // possible method call
        if (lparen.kind == TOK_LPAREN) {
          lparen = lexer_next(allocator, &self->lexer); // method call
...
Stmt parser_visit_for_stmt(Allocator* allocator, Parser* self) {
  ...
  parser_expect(TOK_KW_FOR, lexer_next(&local_allocator, &self->lexer));
...
Stmt parser_visit_if_stmt(Allocator* allocator, Parser* self) {
  ...
  while (true) {
    feed = lexer_peek(&local_allocator, &self->lexer);
    if (feed.kind != TOK_KW_ELSE) {
      break;
    }

    parser_expect(TOK_KW_ELSE, lexer_next(&local_allocator, &self->lexer));

    feed = lexer_peek(&local_allocator, &self->lexer); // maybe has 'if'
...
Stmt parser_visit_assign_stmt(Allocator* allocator, Parser* self) {
  ...
  bool mut = false;
  Token feed = lexer_next(allocator, &self->lexer);
...
Body parser_visit_body(Allocator* allocator, Parser* self) {
  ...
  while (true) {
    Token feed = lexer_peek(&local_allocator, &self->lexer);
    switch (feed.kind) {
      case TOK_KW_TRUE:
      case TOK_KW_FALSE:
      case TOK_INTEGER:
...
Vector parser_visit_module_inner(Allocator* allocator, Parser* self, u8 is_source) {
  ...
  while (true) {
    ...
    allocator_deinit(&local_allocator);
    token = lexer_peek(&local_allocator, &self->lexer);
...
AstNode parser_visit_struct(Allocator* allocator, Parser* self) {
  ...
  feed = lexer_peek(allocator, &self->lexer);
...
AstNode parser_visit_func(Allocator* allocator, Parser* self) {
  ...
  vector_init(allocator, &func.param_vec, sizeof(FuncArg), 1);
  ...
  feed = lexer_next(allocator, &self->lexer);
...
Parser* parser_new(Allocator* allocator, Lexer lexer) {
  Parser* result = allocator_alloc(allocator, sizeof(Parser));
