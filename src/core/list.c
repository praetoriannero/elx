...
void list_init_args(List* self, ListInitArgs* args) { list_init(self, args->alloc, args->item_size); }
...
ListIter list_iter(List* self) { return (ListIter){.list = self, .iter = self->head}; }
