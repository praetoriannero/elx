    ...
    while (list_iter_next(&entry_list_iter, (void**)&entry)) {
        if (self->comp_func(entry->key, key)) {
            return (HashTableResult){
                .entry_list = entry_list, .entry = entry, .entry_idx = entry_idx, .key_loc = key_loc};
        }
        entry_idx++;
    }
...
void hash_table_iter_init(HashTableIter* self, HashTable* table) { _hash_table_iter_init(self, table, 0); }
